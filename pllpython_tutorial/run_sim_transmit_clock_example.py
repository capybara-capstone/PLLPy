...
pll.show(plot_type=None, sim_type='PLL')

pll_jitter = calc.calculate_jitter(
    np.array(pll.output), start_time=12e-6, stop_time=24e-6)[0]

# load data from previous files
h = np.load("./pllpython_tutorial/data_for_serdespy_examples/h_thru.npy")
h_ctle = np.load("./pllpython_tutorial/data_for_serdespy_examples/h_ctle.npy")
ffe_tap_weights = np.load(
    "./pllpython_tutorial/data_for_serdespy_examples/rx_ffe_tap_weights.npy")
dfe_tap_weights = np.load(
    "./pllpython_tutorial/data_for_serdespy_examples/rx_dfe_tap_weights.npy")
tx_fir_tap_weights = np.load(
    "./pllpython_tutorial/data_for_serdespy_examples/tx_fir_tap_weights.npy")
...
TX.gaussian_jitter(stdev_div_UI=pll_jitter)
...
sdp.simple_eye(TX.signal, samples_per_symbol*3, 1000, TX.UI/TX.samples_per_symbol,
               "TX Bandwidth-Limited Eye Diagram (-3dB frequency at 100GHz)")
...
sdp.simple_eye(signal_out_ctle, samples_per_symbol*3, 1000,
               TX.UI/TX.samples_per_symbol, "Eye Diagram with CTLE")

main_cursor = np.max(signal_out_ctle)/np.max(voltage_levels)

RX = sdp.Receiver(signal_out_ctle, samples_per_symbol,
                  nyquist_f, voltage_levels, shift=True, main_cursor=0.5)

# slice signal at centre of eye
RX.slice_signal()

# add noise with standard deviation 0.1
RX.signal_BR = RX.signal_BR + \
    np.random.normal(scale=0.05, size=RX.signal_BR.size)

# plot baud-rate eye diagram with ctle and noise
sdp.simple_eye(RX.signal_BR, 3, 1000, TX.UI,
               "Eye Diagram with CTLE and noise BR")
...
sdp.simple_eye(RX.signal_BR, 3, 800, TX.UI /
               TX.samples_per_symbol, "Eye Diagram after DFE")

# %%check the number of errors and calculate BER
err = sdp.prqs_checker(10, data, RX.symbols_out[10:-10])

print("Bits Transmitted =",
      RX.symbols_out[10:-10].size*2, 'Bit Errors =', err[0])
