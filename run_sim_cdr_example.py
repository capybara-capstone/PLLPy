...
#start CDR
pll.start_cdr(data_in[:settings.sample_count]) #need to truncate or extend to match samples in PLL
...
trigger_val = 0.5
threshold = 0.5
rising_edge = np.flatnonzero((pll_out[:-1] < trigger_val) & (pll_out[1:] > trigger_val))+1
retimed_data = []

for i in range(len(rising_edge)):
    retimed_data.append(sdp.nrz_decision(data_in[rising_edge[i]], threshold))
    
pll.show(plot_type='web', sim_type='CDR', input=data_in[:settings.sample_count])
